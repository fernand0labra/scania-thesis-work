...
from generator.src.modules.diffusion.utils import checkpoint
...
def zero_module(module):  # Zero out the parameters of a module and return it.
...
class GEGLU(nn.Module):  # Feedforward
...
class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, glu=False, dropout=0.):
        ...
        proj_in = nn.Sequential(nn.Linear(dim, inner_dim), 
                                nn.GELU()) if not glu else GEGLU(dim, inner_dim)

        self.net = nn.Sequential(proj_in, 
                                 nn.Dropout(dropout), 
                                 nn.Linear(inner_dim, default(dim_out, dim)))
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        q, k, v = rearrange(self.to_qkv(x), 'b (qkv heads c) h w -> qkv b heads c (h w)', heads = self.heads, qkv=3)
        context = torch.einsum('bhdn,bhen->bhde', k.softmax(dim=-1), v)
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self, x, context=None):
        ...
        x = self.attn2(self.norm2(x), context=context) + x  # Self-attention if context is none
